...
export class ReturnsApi {
  ...
  static async resolveOrderId(orderNumber: string): Promise<number | null> {
    try {
      const res = await axios.get<{ id?: number } | { data?: { id: number } }>(
  ...
  static async submit(orderId: number, payload: {
    reason: Reason;
    description: string;
    contactEmail: string;
  }) {
