...
Tbl(doc, ['Thuộc tính','Chi tiết'], [
    ...
    ['Luồng chính', J(
        '1. POST /api/auth/login với usernameOrEmail + rawPassword.',
...
Tbl(doc, ['Thuộc tính','Chi tiết'], [
    ...
    ['Luồng chính', J(
        '1. POST /api/orders/from-cart; X-Idempotency-Key BẮT BUỘC.',
...
Tbl(doc, ['Thuộc tính','Chi tiết'], [
    ...
    ['Luồng chính', J(
        '1. POST /api/orders/{id}/return-requests.',
...
Tbl(doc, ['Method','Endpoint','Mô tả'], [
    ['POST','/api/auth/register','Đăng ký + Set-Cookie.'],
    ['POST','/api/auth/login','Đăng nhập + Set-Cookie.'],
    ['POST','/api/auth/refresh','Rotation.'],
    ['GET','/api/auth/validate','Validate JWT (legacy).'],
])
P(doc, 'Protected:', bold=True)
Tbl(doc, ['Method','Endpoint','Mô tả','Quyền'], [
    ['POST','/api/auth/logout','Logout','User+'],
    ['POST','/api/auth/change-password','Đổi mật khẩu','User'],
    ['POST','/api/auth/revoke-all','Revoke all','Admin/self'],
    ['POST','/api/auth/users/{id}/lock','LOCK','Admin'],
    ['POST','/api/auth/users/{id}/unlock','UNLOCK','Admin'],
    ['POST','/api/auth/users/{id}/disable','DISABLE','Admin'],
    ['POST','/api/auth/users/{id}/enable','ENABLE','Admin'],
    ['POST','/api/auth/users/{id}/roles/{role}','Gán role','Admin'],
    ['DELETE','/api/auth/users/{id}/roles/{role}','Bỏ role','Admin'],
    ['GET','/api/auth/users/{id}','User by ID','Admin/self'],
    ['GET','/api/auth/users','List users','Admin'],
    ['GET','/api/auth/check-permission','Check permission','Admin'],
    ['GET','/api/auth/check-role','Check role','Admin'],
])
...
Tbl(doc, ['Method','Endpoint','Mô tả'], [
    ...
    ['GET','/api/profiles/me/addresses','Addresses.'],
    ['POST','/api/profiles/me/addresses','Thêm.'],
    ['PUT','/api/profiles/me/addresses/{id}','Update.'],
    ['DELETE','/api/profiles/me/addresses/{id}','Xóa.'],
    ['POST','/api/profiles/me/addresses/{id}/default','Set default.'],
])
...
Tbl(doc, ['Method','Endpoint','Mô tả','Quyền'], [
    ...
    ['POST','/api/products','Tạo SP','Admin/Staff'],
    ['PUT','/api/products/{id}','Update','Admin/Staff'],
    ['GET','/api/products','List','Public'],
    ['GET','/api/products/{id}','Detail','Public'],
    ['GET','/api/products/slug/{slug}','By slug','Public'],
    ['GET','/api/products/search?q=','Search','Public'],
    ['POST','/api/products/batch','Batch IDs','Public'],
    ['POST','/api/variants','Tạo variant','Admin/Staff'],
    ['POST','/api/attrs','Tạo attribute','Admin'],
])

H(doc, '6.5 - 6.14. API các service còn lại', level=2)
Tbl(doc, ['Service','Method','Endpoint','Mô tả'], [
    ['Inventory','GET','/api/inventory/{variantId}','Xem stock'],
    ['Inventory','POST','/api/inventory/reserve','Giữ hàng (System)'],
    ['Inventory','POST','/api/inventory/release','Release'],
    ['Inventory','POST','/api/inventory/confirm','Confirm'],
    ['Cart','GET','/api/cart','Xem giỏ'],
    ['Cart','POST','/api/cart/items','Thêm item (idem)'],
    ['Cart','PUT','/api/cart/items/{id}','Update qty'],
    ['Cart','DELETE','/api/cart/items/{id}','Xóa item'],
    ['Order','POST','/api/orders/from-cart','Tạo đơn (idem)'],
    ['Order','GET','/api/orders/{id}','Chi tiết'],
    ['Order','GET','/api/orders','User orders'],
    ['Order','GET','/api/admin/orders','All orders (Admin)'],
    ['Order','POST','/api/orders/{id}/cancel','Hủy'],
    ['Order','POST','/api/orders/{id}/return-requests','Yêu cầu trả'],
    ['Order','POST','/api/return-requests/{id}/approve','Duyệt (Admin)'],
    ['Order','POST','/api/return-requests/{id}/complete','Hoàn tất'],
    ['Payment','GET','/api/payments/{id}','Chi tiết'],
    ['Payment','GET','/api/payments/by-order/{orderId}','Theo order'],
    ['Payment','POST','/api/payments/webhook/sepay','SePay webhook (HMAC)'],
    ['Payment','POST','/api/payments/{id}/refund','Refund (Admin)'],
    ['Shipping','POST','/api/shipments','Tạo (System)'],
    ['Shipping','GET','/api/shipments/{id}','Chi tiết + tracking'],
    ['Shipping','POST','/api/shipments/webhook/ghn','GHN webhook'],
    ['Voucher','POST','/api/vouchers','Tạo (Admin)'],
    ['Voucher','GET','/api/vouchers','Active list'],
    ['Voucher','POST','/api/vouchers/validate','Validate (System)'],
    ['Voucher','POST','/api/vouchers/redeem','Apply (System)'],
    ['Voucher','POST','/api/vouchers/release','Compensation'],
    ['Flash Sale','POST','/api/flash-sales','Tạo (Admin)'],
    ['Flash Sale','GET','/api/flash-sales/active','Active'],
    ['Flash Sale','GET','/api/flash-sales/{id}/availability','Quota còn'],
    ['Flash Sale','POST','/api/flash-sales/{id}/participate','Tham gia (Lua)'],
    ['Notification','GET','/api/notifications','List user'],
    ['Notification','POST','/api/notifications/{id}/read','Mark read'],
    ['Notification','GET','/api/notifications/sse','SSE push'],
    ['Review','POST','/api/reviews','Tạo (đã mua)'],
    ['Review','GET','/api/reviews/product/{id}','By product'],
    ['Review','POST','/api/reviews/{id}/approve','Duyệt (Admin)'],
    ['Wishlist','POST','/api/wishlist','Thêm'],
    ['Wishlist','GET','/api/wishlist','Xem list'],
    ['Search','GET','/api/search?q=&category=','Filter search'],
    ['Search','GET','/api/search/autocomplete','Gợi ý'],
])
page_break(doc)
print("Done Ch6")
...
Tbl(doc, ['Endpoint','Key','TTL','Behavior'], [
    ['POST /api/orders/from-cart','X-Idempotency-Key','24h','Trả OrderDTO cũ.'],
    ['POST /api/payments','X-Idempotency-Key','24h','Trả Payment cũ.'],
    ['POST /api/payments/{id}/refund','X-Idempotency-Key','24h','Trả Refund cũ.'],
    ['POST /api/cart/items','X-Idempotency-Key','24h','Trả Cart cũ.'],
...
Tbl(doc, ['Endpoint','Limit','Window','Implementation'], [
    ['/api/auth/login','5/IP','15p','Bucket4j + Redis'],
    ['/api/auth/register','3/IP','60p','Bucket4j'],
    ['/api/auth/refresh','20/user','5p','Bucket4j'],
    ['/api/orders/from-cart','10/user','60p','Bucket4j'],
    ['/api/* (mọi endpoint)','100/IP','60s','Redis counter'],
...
Code(doc, J(
    ...
    '          predicates: [Path=/api/products/**,/api/categories/**]',
    '        - id: cart',
    '          uri: lb://CART-SERVICE',
    '          predicates: [Path=/api/cart/**]',
    '        - id: order',
    '          uri: lb://ORDER-SERVICE',
    '          predicates: [Path=/api/orders/**,/api/return-requests/**]',
    '        - id: payment',
