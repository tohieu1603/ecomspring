...
REG=$(curl -s -X POST "$GW/api/auth/register" \
...
# ─── TC2: Login ───────────────────────────────────────────────────────
info "TC2: Login"
curl -s -c "$COOKIES" -X POST "$GW/api/auth/login" \
    -H "Content-Type: application/json" \
...
# ─── TC3: /me ─────────────────────────────────────────────────────────
info "TC3: /api/auth/me"
ME=$(curl -s -b "$COOKIES" "$GW/api/auth/me")
echo "$ME" | grep -q "$EMAIL" && ok "TC3 Identity returned" || fail "TC3 /me" "$ME"

# ─── TC4: Browse catalog ──────────────────────────────────────────────
info "TC4: Browse products"
HTTP=$(curl -s -o /dev/null -w "%{http_code}" "$GW/api/products?page=0&size=5")
...
CAT=$(curl -s -b "$COOKIES" -X POST "$GW/api/categories" \
    -H "Content-Type: application/json" \
...
SKU="E2E-SKU-$TS"
PROD=$(curl -s -b "$COOKIES" -X POST "$GW/api/products" \
    -H "Content-Type: application/json" \
...
ADD=$(curl -s -b "$COOKIES" -X POST "$GW/api/cart/items" \
    -H "Content-Type: application/json" \
...
UPD=$(curl -s -b "$COOKIES" -X PUT "$GW/api/cart/items/$VAR_ID" \
...
ORDER=$(curl -s -b "$COOKIES" -X POST "$GW/api/orders" \
    -H "Content-Type: application/json" \
...
if [[ -n "$ORDER_ID" ]]; then
    ...
    DETAIL=$(curl -s -b "$COOKIES" "$GW/api/orders/$ORDER_ID")
...
sleep 4
HTTP=$(curl -s -o /dev/null -w "%{http_code}" -b "$COOKIES" "$GW/api/notifications/my/feed?size=5")
...
sleep 6
SEARCH=$(curl -s "$GW/api/search?q=E2E&size=10")
...
if [[ -z "$SEARCH_HITS" ]]; then
    ...
    INDEX=$(curl -s -b "$COOKIES" -X POST "$GW/api/search/index" \
        -H "Content-Type: application/json" \
...
SUGGEST=$(curl -s "$GW/api/search/suggest?q=e2e&size=5")
...
VCODE="E2E${TS}"
CREATE_V=$(curl -s -b "$COOKIES" -X POST "$GW/api/vouchers" \
    -H "Content-Type: application/json" \
...
if [[ -z "$VID" ]]; then
    ...
    VAL=$(curl -s -b "$COOKIES" -X POST "$GW/api/vouchers/validate" \
        -H "Content-Type: application/json" \
...
REL=$(curl -s -b "$COOKIES" -X POST "$GW/api/vouchers/release" \
    -H "Content-Type: application/json" \
    -d "{\"code\":\"$VCODE\",\"orderId\":\"E2E-${TS}\"}")
REL_HTTP=$(curl -s -o /dev/null -w "%{http_code}" -b "$COOKIES" -X POST "$GW/api/vouchers/release" \
    -H "Content-Type: application/json" \
    -d "{\"code\":\"$VCODE\",\"orderId\":\"E2E-${TS}\"}")
